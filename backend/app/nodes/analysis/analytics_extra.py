...
class DatasetMergeNode(BaseNode):
    description = NodeDescription(
        ...
        icon="git-merge",
        description="Join a secondary dataset onto the current data using an inner, left, or outer join on a shared column",
        color="#f97316",
        inputs=["main"],
        outputs=["main"],
        properties=[
...
class FeatureTransformNode(BaseNode):
    description = NodeDescription(
        ...
        category="ML Analysis",
        icon="shuffle",
        description="Scale numeric columns of a dataset using standard, min-max, or log1p transformations",
        color="#f97316",
        inputs=["main"],
        outputs=["main"],
        properties=[
...
class DriftMonitorNode(BaseNode):
    description = NodeDescription(
        ...
        category="ML Analysis",
        icon="gauge",
        description="Compare incoming data against a reference dataset to detect distribution drift above a threshold",
        color="#f97316",
        inputs=["main"],
        outputs=["main"],
        properties=[
...
class TimeSeriesStudioNode(BaseNode):
    description = NodeDescription(
        ...
        category="ML Analysis",
        icon="line-chart",
        description="Extract and inspect a time series from the date and value columns of a dataset",
        color="#14b8a6",
        inputs=["main"],
        outputs=["main"],
        properties=[
...
class FFTAnalysisNode(BaseNode):
    description = NodeDescription(
        ...
        icon="activity",
        color="#14b8a6",
        description="Run a fast Fourier transform on a numeric column to surface frequency patterns.",
        inputs=["main"],
        outputs=["main"],
...
class MarketBasketNode(BaseNode):
    description = NodeDescription(
        ...
        icon="shopping-cart",
        color="#f97316",
        description="Mine association rules from basket/transaction data.",
        inputs=["main"],
        outputs=["main"],
        properties=[
